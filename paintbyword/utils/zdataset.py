...
def truncated_z_sample(size, depth, seed=1, truncation=1.0, device=None):
    z = truncnorm.rvs(
        -2 * truncation, 2 * truncation, size=(size, depth), random_state=seed,
    ).astype(numpy.float32)
    return torch.from_numpy(z, device='cpu' if device is None else device)


def truncated_z_dataset(model, size=100, seed=1, truncation=1.0):
    if hasattr(model, 'input_shape'):
        z_dim = model.input_shape[1]
    elif hasattr(model, 'z_dim'):
        z_dim = model.z_dim
    elif hasattr(model, 'dim_z'):
        z_dim = model.dim_z
    else:
        raise ValueError('Could not determine z dim size...')
    return TensorDataset(
        truncated_z_sample(size, z_dim, seed=seed, truncation=truncation)
    )
...
def training_loader(z_generator, batch_size, loader_size=10000):
    ...
    g_epoch = 1
    while True:
        z_data = z_dataset_for_model(z_generator, size=epoch_size, seed=g_epoch + 1)
