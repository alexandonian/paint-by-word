        ...
        def callback(module, input, output):
            if self.fn is not None:
                output = invoke_with_optional_args(
                    self.fn, module=module, input=input, output=output,
                )
                return output
...
class InstrumentedModel(torch.nn.Module):
    ...
    def retain_layer(self, layername, detach=None, retain_grad=None, clone=None):
        ...
        '''
        self.retain_layers(
            [layername], detach=detach, retain_grad=retain_grad, clone=clone
        )
    ...
    def _postprocess_forward(self, x, aka):
        ...
        if aka in self._retained:
            self._retained[aka] = recursive_copy(
                x,
...
def hierarchical_subsequence(
    ...
    if hasattr(sequential, 'training'):
...
def concatenate_sequence(*args):
    '''
    Creates a pytorch Sequential model by concatenating the
    immediate children of multiple Sequentials or arrays of
    modules.  Renumbers unnamed chlidren; preserves names of
    named chlidren; and triggers an assert if a name is
    duplicated.
    '''
    children = []
    seen_names = set()
    next_integer = 0
    def next_new_integer():
        while str(next_integer) in seen_names:
            next_integer += 1
        result = str(next_integer)
        next_integer += 1
        return result
    for seq in args:
        if isinstance(seq, torch.nn.Sequential):
            gen = seq._modules.items()
        elif isinstance(seq, dict):
            gen = seq.items()
        elif isinstance(seq, list):
            gen = seq
        elif isinstance(seq, (torch.nn.Module, tuple)):
            gen = [seq]
        else:
            assert False, f'{type(seq)} cannot be concatenated'
        for item in gen:
            if isinstance(item, (tuple, list)):
                assert len(item) == 2
                name, item = item
            else:
                name = next_new_integer()
            if name in seen_names:
                if name.isdigit():
                    name = next_new_integer()
                assert False, f'name {name} was repeated'
            assert isinstance(item, torch.nn.Module)
            children.append((name, item))
    result = torch.nn.Sequential(OrderedDict(children))
    for _, child in children:
        if hasattr(child, 'training'):
            result.training = child.training
            break
    return result
