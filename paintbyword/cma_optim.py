...
class CMA:
    ...
    _init_idx = 0  # which example index to initalize CMA if params is batched
    ...
    def __init__(
        ...
        self.params_concat = np.hstack(
            [p.view(p.size(0), -1)[:1].detach().cpu() for p in params.values()]
        )
    ...
    def step(self):
        cma_results = torch.tensor(self.cmaes_optim.ask(), dtype=torch.float32).to(self.device)
