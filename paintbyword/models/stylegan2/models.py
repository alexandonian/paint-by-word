    ...
    def load_state_dict(self, data, latent_avg=None, **kwargs):
        ...
        for k, v in data.items():
            ...
            if self.mconv == 'seq':
                ...
                k = re.sub(r'rgb\.conv\.weight$', 'rgb.conv.dconv.weight', k)
            else:
                k = re.sub(r'mconv\.dconv\.weight$', 'mconv.weight', k)
                k = re.sub(r'rgb\.conv\.dconv\.weight$', 'rgb.conv.weight', k)
            newdata[k] = v
        ...
        if 'latents.latent_avg' in newdata:
            self.latents.latent_avg.resize_(newdata['latents.latent_avg'].shape)
...
class DemodulatedConv2dF(nn.Module):
    ...
    def forward(self, d):
        ...
        return DataBag(d, **{self.return_as: out})
...
class ModulatedConv2dF(ModulatedConv2d):
    ...
    def forward(self, d):
        return DataBag(d, **{self.return_as: super().forward(d.fmap, d.style)})
...
class AdjustLatent(nn.Module):
    def __init__(self, n_latent, truncation=1.0):
        ...
        self.n_latent = n_latent
        self.truncation = truncation
        self.register_buffer('latent_avg', torch.tensor([]))

    def forward(self, d):
        if self.truncation != 1.0 and len(self.latent_avg) > 0:
...
class ToRGBF(nn.Module):
    def __init__(
        ...
        self.conv = MConv(
            in_channel, 3, 1, style_dim, demodulate=False, return_as='output'
        )
